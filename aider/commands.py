...
"""
    ...
    def cmd_ask(self, args):
        ...
        if self.coder.ask_model:
            try:
                self.coder.check_ask_model_capabilities()
                self.coder.show_announcements()
                original_model = self.coder.main_model
                original_api_key = self.coder.main_model.api_key
                self.coder.main_model = models.Model(self.coder.ask_model)
                self.coder.main_model.api_key = self.coder.ask_api_key
                result = self._generic_chat_command(args, "ask")
                self.coder.main_model = original_model
                self.coder.main_model.api_key = original_api_key
                return result
            except Exception as e:
                self.coder.io.tool_error(f"Error using custom ask model: {e}")
                self.coder.io.tool_output("Falling back to the main model.")
